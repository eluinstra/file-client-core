...
public class Sha256Checksum implements ValueObject<String>
{
	...
	public Sha256Checksum(final byte[] checksum)
	{
	...
	{
		value =
				Try.success(checksum).andThen(v -> inclusiveBetween(32, 64, v.length())).map(String::toUpperCase).andThen(v -> matchesPattern(v, "^[0-9A-F]*$")).get();
	}
